...
class ClosingCostCalculationsPhpResource(SyncAPIResource):
    ...
    def calculate(
        ...
        *,
        ...
        app_mods: Iterable[int] | Omit = omit,
        client_id: int | Omit = omit,
        close_date: Union[str, date] | Omit = omit,
        doc_type: closing_cost_calculations_php_calculate_params.DocType | Omit = omit,
        exdebt: float | Omit = omit,
        filename: str | Omit = omit,
        include_appraisal: int | Omit = omit,
        include_full_policy_amount: int | Omit = omit,
        include_payee_info: int | Omit = omit,
        include_pdf: int | Omit = omit,
        include_property_tax: int | Omit = omit,
        include_section: int | Omit = omit,
        include_seller_responsible: int | Omit = omit,
        int_name: str | Omit = omit,
        loan_amount: float | Omit = omit,
        loan_info: LoanInfoParam | Omit = omit,
        loanpol_level: Literal[1, 2] | Omit = omit,
        owners_level: Literal[1, 2] | Omit = omit,
        prior_insurance: float | Omit = omit,
        prior_insurance_date: Union[str, date] | Omit = omit,
        purchase_price: float | Omit = omit,
        qst: Dict[str, str] | Omit = omit,
        request_endos: SequenceNotStr[str] | Omit = omit,
